...
def lnlike_asymm(theta, x, y, yerr, resolution, variables, model_function, mask_data, xerr=None,debug=False):
    ...
    for i in range(len(x)):
        mask = x < y_model
        yerr_i = yerr[i][1] # 0 is the low error bar, 1 is the high error bar
        yerr_i[mask] = yerr[i][0]
        ...
        if yerr[i] is None:
            ...
            lnlike += -0.5*(np.sum( (y[i] - y_model[i])**2/yerr_i**2 + np.log(2*np.pi*yerr_i**2) + log_xerr)) 
...
def lnprob_asymm(theta, x, y, yerr, resolution, variables, variables_order, model_function, mask_data, xerr=None, debug=False):
